...
async function generateMerkleProof(deposit) {
  ...
  const fromBlockNum = (await web3.eth.getBlock('latest')).number - 1000
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  ...
  if (relayerURL) {
    ...
    await tornado.methods
      .withdraw(proof, args[0], args[1],args[2], args[3], args[4], args[5])
      .send({ from: senderAccount, value: refund.toString(), gas: 10e6 })
...
async function init({ rpc, noteNetId, currency = 'dai', amount = '100' }) {
  ...
  if (inBrowser) {
    ...
    contractJson = await (await fetch('build/contracts/ETHTornado.abi.json')).json()
    ...
    contractJson = require('./build/contracts/ETHTornado.abi.json')
  ...
  isLocalRPC = netId > 42 && netId !== 80001 && netId !== 137
  ...
  tornado = new web3.eth.Contract(contractJson, tornadoInstance)
